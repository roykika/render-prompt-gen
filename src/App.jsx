...
const PROGRAM_TYPES = {
  residential:{label:"Residential",icon:"⌂",desc:"Apartment building"},
  office:{label:"Office",icon:"▣",desc:"Office building"},
  hotel:{label:"Hotel",icon:"◈",desc:"Hotel building"},
  mixed:{label:"Mixed-Use",icon:"⊞",desc:"Mixed-use building"},
};

// Floor use presets per program type
const FLOOR_USE_PRESETS = {
  residential: [
    "Standard apartments — 3-5 room units",
    "Luxury apartments — large 4-6 room units",
    "Penthouse floor — full-floor unit, terrace",
    "Sky amenity floor — gym, pool, lounge",
    "Roof deck — green roof, pergola, seating",
    "Duplex units — upper two floors",
    "Studio / micro-apartments",
    "Affordable housing — compact efficient units",
    "Senior living units",
    "Short-term rental units (Airbnb-style)",
  ],
  office: [
    "Open-plan corporate office floor",
    "Co-working / flexible workspace floor",
    "Executive floor — private offices, boardroom",
    "Sky lobby — reception, meeting rooms with views",
    "Server room / technical floor",
    "Café / staff amenity floor",
    "Conference center floor",
    "Trading floor — dense workstations",
    "Creative studio floor — high ceilings, open",
    "Medical / clinic floor",
  ],
  hotel: [
    "Standard hotel rooms floor",
    "Superior rooms — larger, city views",
    "Suite floor — junior and senior suites",
    "Presidential suite — full floor",
    "Spa & wellness floor",
    "Sky bar & restaurant floor — panoramic views",
    "Infinity pool deck floor",
    "Meeting & events floor — ballroom",
    "Back-of-house / staff floor",
    "Executive lounge floor",
  ],
  mixed: [
    "Ground floor retail — shops, café, pharmacy",
    "Retail podium — 2-3 floors of commercial",
    "Office mid-zone floors",
    "Residential upper floors",
    "Hotel floors (upper zone)",
    "Food & beverage floor — restaurants",
    "Cultural / public floor — gallery, library",
    "Parking podium — concealed behind active frontage",
    "Sky amenity — shared between uses",
    "Mechanical / technical floor (hidden)",
  ],
};
const SEASONS = {
...
const PRESETS = {
  ...
  frame:["White precast concrete","Off-white GRC panels","Dark charcoal concrete","Travertine stone cladding","Terracotta ceramic panels","Burnished bronze anodized aluminum","Pale limestone","Brnowitz system — cast-in-place insulated concrete","ECOWALL lightweight steel frame system","Curtain wall — aluminum and glass facade system"],
...
const CONTEXT_PROFILES = {
  tlv_center: {
    label:"Tel Aviv Center", icon:"🏙", flag:"🇮🇱",
    location:"Tel Aviv city center, Israel",
    streetContext:"Dense Israeli urban street context, Dizengoff-style boulevard",
    background:"Tel Aviv skyline with Azrieli towers visible on horizon",
    foreground:"Wide landscaped boulevard with mature ficus trees, pedestrians, parked cars, outdoor cafés",
    neighborBuildings:"Mixed 6-12 story residential buildings from 1970s-2000s, 40% transparent facades, varied setbacks",
    crowdDensity:"Moderate everyday urban activity",
    activityType:"Mixed casual urban crowd — coffee culture, cyclists, young professionals",
    skyDescription:"Deep blue Mediterranean sky, scattered cumulus clouds",
    colorTemp:"5200K — bright Mediterranean daylight",
    treeType:"Mature Ficus benjamina, Phoenix dactylifera date palms",
    sidewalk:"Wide light concrete sidewalk with subtle expansion joints, 4m width",
    roadWidth:"Wide 4-lane boulevard with central planted median",
  },
  petah_tikva: {
    label:"Petah Tikva", icon:"🏘", flag:"🇮🇱",
    location:"Petah Tikva, Israel",
    streetContext:"Dense Israeli suburban street context",
    background:"Low-rise suburban fabric, water tower visible in distance",
    foreground:"Tree-lined street with parked cars, few pedestrians, local shops at ground level",
    neighborBuildings:"7-12 story residential blocks from 1980s-2010s, concrete facade with balconies, 20% glazing",
    crowdDensity:"Sparse — few distant figures",
    activityType:"Local residents, families with children, delivery vehicles",
    skyDescription:"Clear azure Mediterranean sky — no clouds",
    colorTemp:"5500K — strong Israeli summer sun",
    treeType:"Jacaranda, Ficus, Eucalyptus",
    sidewalk:"Standard concrete sidewalk, 2.5m width, minor weathering",
    roadWidth:"Standard 2-lane residential street with parking lane",
  },
  herzliya: {
    label:"Herzliya Pituach", icon:"🌊", flag:"🇮🇱",
    location:"Herzliya Pituach, Israel",
    streetContext:"Upscale coastal business and residential district",
    background:"Mediterranean sea on horizon, marina visible",
    foreground:"Reflecting pool in plaza, manicured landscaping, luxury vehicles",
    neighborBuildings:"Modern glass office towers and luxury residential 12-25 story, curtain wall facades, highly transparent",
    crowdDensity:"Sparse — few distant figures",
    activityType:"Business professionals, luxury vehicles, joggers on promenade",
    skyDescription:"Clear azure sky — no clouds, sea breeze haze at horizon",
    colorTemp:"5800K — coastal bright light with slight sea haze",
    treeType:"Washingtonia palms, ornamental olive trees",
    sidewalk:"Premium granite pavers, 5m width",
    roadWidth:"Wide divided boulevard with landscaped median",
  },
  jerusalem: {
    label:"Jerusalem Hills", icon:"⛪", flag:"🇮🇱",
    location:"Jerusalem, Israel",
    streetContext:"Historic Jerusalem stone city context",
    background:"Jerusalem hills, stone terraces, distant minarets and church spires",
    foreground:"Narrow stone-paved lane, few pedestrians in traditional dress mix with modern",
    neighborBuildings:"2-6 story Jerusalem stone clad buildings, arched windows, dense historic fabric",
    crowdDensity:"Moderate everyday urban activity",
    activityType:"Mixed religious and secular pedestrians, tour groups, local shoppers",
    skyDescription:"Clear azure sky — golden Jerusalem light, slightly warm",
    colorTemp:"4800K — warm Jerusalem golden hour quality even at midday",
    treeType:"Cypress, olive trees, stone pine",
    sidewalk:"Jerusalem stone pavers, irregular historic pattern",
    roadWidth:"Narrow 2-lane street, often one-way",
  },
  ramat_gan: {
    label:"Ramat Gan / Diamond District", icon:"💎", flag:"🇮🇱",
    location:"Ramat Gan, Israel",
    streetContext:"Dense Israeli business district — Bursa / Diamond Exchange area",
    background:"Ramat Gan skyline with Moshe Aviv Tower, Tel Aviv towers on horizon",
    foreground:"Wide landscaped boulevard with mature ficus trees, business pedestrians, parked cars",
    neighborBuildings:"Mix of 1970s-80s low commercial and modern 20-40 story glass office towers",
    crowdDensity:"Busy lively street scene",
    activityType:"Business professionals, diamond merchants, lunch crowds, delivery trucks",
    skyDescription:"Dramatic sky with scattered cumulus, urban heat shimmer at street level",
    colorTemp:"5400K — clear urban daylight",
    treeType:"Mature Ficus benjamina, ornamental palms",
    sidewalk:"Wide commercial sidewalk with street furniture, 5m width",
    roadWidth:"Wide 4-lane commercial boulevard",
  },
};

// ─── SHOT TYPES ───────────────────────────────────────────
const SHOT_TYPES = {
  hero:    {label:"Hero Shot",    icon:"🎬", desc:"Wide establishing — full building in context", angle:"Low street-level perspective, 35mm lens, slight upward tilt", comp:"Wide-angle hero — all buildings with urban context", framing:"60% front facade / 40% side facade"},
  detail:  {label:"Detail Shot",  icon:"🔍", desc:"Close facade — material, joint, texture", angle:"Eye-level close-up, 85mm lens, perfectly level", comp:"Tight framing on facade detail — material texture and shadow depth", framing:"100% front facade, 1-3 floors visible"},
  human:   {label:"Human Scale",  icon:"👤", desc:"Street level — people in foreground, building behind", angle:"Eye-level pedestrian view, 35mm lens, perfectly level", comp:"People in foreground at 1/3, building filling background", framing:"50% street life / 50% building"},
  aerial:  {label:"Aerial 45°",   icon:"🚁", desc:"Bird's eye — massing, roof, context", angle:"Bird's eye 45° aerial, 28mm lens, slight downward tilt", comp:"Panoramic showing full site and immediate surroundings", framing:"Building fills 60% of frame, context 40%"},
  dusk:    {label:"Dusk Shot",    icon:"🌆", desc:"Interior glow vs exterior — magic hour", angle:"Eye-level pedestrian view, 35mm lens, perfectly level", comp:"Wide-angle showing facade lit from within against dusk sky", framing:"Full building height, street activity silhouetted"},
  planning:{label:"Planning Shot", icon:"📋", desc:"Technical — setbacks, height, compliance", angle:"Elevated 3-point view from across boulevard, 50mm lens", comp:"Full building with visible setbacks and site boundaries", framing:"Full building + site context, technical framing"},
};
...
const ISRAEL_CITIES = {
  ...
  "Haifa":       {lat:32.82, lon:35.0},
  "Herzliya":    {lat:32.17, lon:34.84},
  "Ramat Gan":   {lat:32.08, lon:34.82},
  "Petah Tikva": {lat:32.09, lon:34.88},
  "Beer Sheva":  {lat:31.25, lon:34.79},
};
const MONTHS = ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];

function calcSun(cityKey, month, hour) {
  const city = ISRAEL_CITIES[cityKey] || ISRAEL_CITIES["Tel Aviv"];
  ...
  const cosAz = (Math.sin(declination) - Math.sin(lat)*sinAlt) / (Math.cos(lat)*Math.cos(Math.asin(sinAlt)));
  let azimuth = Math.acos(Math.max(-1,Math.min(1,cosAz))) * 180 / Math.PI;
  if (hour > 12) azimuth = 360 - azimuth;
  const shadowLen = elevation > 2 ? (1/Math.tan(elevation*Math.PI/180)).toFixed(1) : "∞";
  let colorTemp = 5500;
  if (hour < 8 || hour > 17) colorTemp = 3200;
  else if (hour < 9 || hour > 16) colorTemp = 4000;
  else if (hour < 10 || hour > 15) colorTemp = 4800;
  return {elevation: elevation.toFixed(1), azimuth: azimuth.toFixed(0), shadowLen, colorTemp};
}

// ─── GROUND FLOOR OPTIONS ─────────────────────────────────
const GROUND_FLOOR_TYPES = ["Standard lobby entrance","Double-height retail (6.6m)","Triple-height grand lobby (9m)","Semi-public colonnade + retail","Parking podium with planted deck","Activated retail frontage — full glazing","Arcade / covered walkway"];
const COLONNADE_TYPES = ["Round columns Ø20cm","Square columns 25×25cm","Rectangular columns 30×60cm","Steel I-beam columns","No colonnade — flush facade","Deep loggia — 6m recess"];
const GROUND_SIGNAGE = ["Café / restaurant signs, backlit","Retail store signage — metal panel","Pharmacy + supermarket + café mix","Bank + insurance offices","No signage — residential lobby only","Mixed retail: electronics, sports, market"];
const INTERIOR_LIGHT_TYPES = ["Warm residential 2700K — life visible","Cool office 4000K — work visible","Warm retail 3000K — products lit","Mixed: warm lower / cool upper","Dark — minimal interior visibility"];

// ─── IMPERFECTIONS ────────────────────────────────────────
const IMPERFECTION_LEVELS = {
  none:   {label:"New Build",    icon:"✨", desc:"Perfect, just completed", text:""},
  light:  {label:"1–3 Years",   icon:"🏗", desc:"Subtle weathering", text:"Subtle panel joint shadowing (8mm reveals), micro irregularity in panel flatness, very slight glass reflectivity variation, minimal dirt at slab edges"},
  medium: {label:"5–10 Years",  icon:"🏢", desc:"Natural aging", text:"Panel joint shadowing, light weathering near ground level, mild dirt accumulation at slab edges and drainage paths, glass reflections include faint urban surroundings, subtle oxidation on aluminum profiles"},
  heavy:  {label:"15–30 Years", icon:"🏚", desc:"Mature urban patina", text:"Visible weathering and patina on lower facade, algae growth near downpipes, concrete spalling at exposed edges, glass deposition streaks, varied aluminum oxidation, repair patches visible, AC unit brackets and pipes"},
};

// ─── PEOPLE & MOTION ──────────────────────────────────────
const MOTION_BLUR_OPTIONS = ["Sharp freeze — 1/1000s","Slight motion — 1/250s","Soft motion blur — 1/60s (long exposure feel)","Strong motion blur — 1/15s","Ghosting — very long exposure"];
const DEMOGRAPHIC_OPTIONS = ["Young urban professionals — 25–35","Families with children — residential feel","Mixed age — authentic neighborhood","Business district — suits and laptops","Students and young creatives","Elderly residents — quiet street","Tourist mix — cameras and maps"];

// ─── MATERIAL SPECS ───────────────────────────────────────
const RAL_PRESETS = [
  {code:"RAL 9010",name:"Pure White",hex:"#F4F0E4"},{code:"RAL 9003",name:"Signal White",hex:"#ECF0F1"},
  {code:"RAL 7015",name:"Slate Grey",hex:"#5B6272"},{code:"RAL 7016",name:"Anthracite Grey",hex:"#383E42"},
  {code:"RAL 7021",name:"Black Grey",hex:"#2B2B2B"},{code:"RAL 9005",name:"Jet Black",hex:"#1A1A1A"},
  {code:"RAL 8017",name:"Chocolate Brown",hex:"#5B3427"},{code:"RAL 6009",name:"Fir Green",hex:"#2D4427"},
  {code:"RAL 3009",name:"Oxide Red",hex:"#6B3028"},{code:"RAL 1015",name:"Light Ivory",hex:"#E8DAB4"},
  {code:"RAL 1013",name:"Oyster White",hex:"#EBE0CC"},{code:"RAL 7044",name:"Silk Grey",hex:"#B8B5AC"},
];
const GLASS_TYPES = [
  {label:"Clear low-iron, 10% reflectivity, subtle blue tint, IOR 1.52","value":"clear low-iron glass with 10% reflectivity, subtle clear light-blue tint, physically accurate IOR 1.52, Fresnel reflections"},
  {label:"Bronze tinted, 20% reflectivity","value":"warm bronze-tinted glass, 20% reflectivity, low-E coating, soft warm interior glow visible"},
  {label:"Mirror high-reflectivity, 40%+","value":"mirror-finish reflective glass, 40%+ reflectivity, strong sky and urban reflections"},
  {label:"Fritted/printed, translucent","value":"ceramic fritted glass, partially translucent, geometric silk-screen pattern, diffuse light transmission"},
  {label:"Dark grey, 15% reflectivity","value":"dark grey tinted glass, 15% reflectivity, minimal interior visibility, urban anonymous look"},
];
...
function TierSelector({ value, onChange, isDark, customFloors, onCustomFloors }) {
  return (
    <div>
      <div className="tier-row">{Object.entries(HEIGHT_TIERS).map(([k, tier]) => { const tc = tier[isDark ? "dark" : "light"]; return (<button key={k} className={"tier-btn" + (value === k ? " on" : "")} style={{"--tc":tc.color,"--tglow":tc.glow}} onClick={() => onChange(k)}><span className="tier-icon">{tier.icon}</span><div className="tier-lbl">{tier.label}</div><div className="tier-fl">{tier.floors}</div></button>); })}</div>
      <div style={{display:"flex",alignItems:"center",gap:8,marginTop:8,padding:"7px 10px",background:"rgba(139,92,246,0.06)",border:"1px solid rgba(139,92,246,0.18)",borderRadius:8}}>
        <span style={{fontSize:11,color:"#A78BFA",fontFamily:"'Syne',sans-serif",fontWeight:600,letterSpacing:".1em",textTransform:"uppercase",whiteSpace:"nowrap"}}>קומות / Floors</span>
        <input
          type="number" min="1" max="120"
          value={customFloors}
          onChange={e => onCustomFloors(e.target.value)}
          placeholder="e.g. 12"
          style={{width:70,background:"transparent",border:"1px solid rgba(139,92,246,0.3)",borderRadius:6,padding:"4px 8px",color:"#ECE8FA",fontSize:13,fontFamily:"'Syne',sans-serif",fontWeight:700,textAlign:"center",outline:"none"}}
        />
        {customFloors && <span style={{fontSize:11,color:"#6A6585"}}>floors — overrides category</span>}
      </div>
    </div>
  );
}
function BuildingForm({ b, onChange, isDark, tr }) {
  ...
  return (<div style={{display:"flex", flexDirection:"column", gap:10}}><div><div className="of-label" style={{marginBottom:7}}>{tr.heightCategory}</div><TierSelector value={b.tier} onChange={setTier} isDark={isDark} customFloors={b.customFloors||""} onCustomFloors={v => onChange({...b, customFloors:v})}/></div><OptionField label={tr.shapeForm} value={b.shape} onChange={v => onChange({...b, shape:v})} options={tier.shapes}/><OptionField label={tr.facadeLang} value={b.facade} onChange={v => onChange({...b, facade:v})} options={tier.facades} multiline/></div>);
}
const defBldg = i => ({id:i, name:B_NAMES[i], tier:"high", shape:HEIGHT_TIERS.high.shapes[0], facade:HEIGHT_TIERS.high.facades[0]});
function BuildingConfig({ buildings, setBuildings, count, setCount, mode, setMode, podium, setPodium, cf, setCF, isDark, t, tr }) {
  ...
  return (
    <div className="sec">
      ...
      {mode === "all" && (<div className="uniform-card"><div className="of-label" style={{marginBottom:8}}>Shared — all {count} building{count>1?"s":""}</div><BuildingForm b={uniform} onChange={b => { setUniform(b); setBuildings(p => p.map(x => ({...x, tier:b.tier, shape:b.shape, facade:b.facade, customFloors:b.customFloors||""}))); }} isDark={isDark} tr={tr}/></div>)}
...
// ─── SUN CALCULATOR COMPONENT ─────────────────────────────
function SunCalculator({ sunData, setSunData, t }) {
  const [city, setCity] = useState("Tel Aviv");
  const [month, setMonth] = useState(7);
  const [hour, setHour] = useState(14);
  const sun = calcSun(city, month, hour);
  const apply = () => {
    const dirs = ["N","NNE","NE","ENE","E","ESE","SE","SSE","S","SSW","SW","WSW","W","WNW","NW","NNW"];
    const az = parseInt(sun.azimuth);
    const dirIdx = Math.round(az / 22.5) % 16;
    const dirLabel = dirs[dirIdx];
    const elevLabel = sun.elevation < 15 ? "Low " + sun.elevation + "° — long raking shadows" :
                      sun.elevation < 35 ? "Medium " + sun.elevation + "° — balanced shadows" :
                      sun.elevation < 55 ? "High " + sun.elevation + "° — moderate shadow depth" :
                      "Very high " + sun.elevation + "° — near-vertical light, short shadows";
    setSunData({
      sunAngle: elevLabel + ", azimuth " + az + "° (" + dirLabel + ")",
      colorTemp: sun.colorTemp + "K",
      shadowLen: sun.shadowLen + "x building height shadow length",
      city, month, hour
    });
  };
  const elevNum = parseFloat(sun.elevation);
  const isNight = elevNum < 0;
  return (
    <div style={{background:"rgba(251,191,36,0.06)",border:"1px solid rgba(251,191,36,0.2)",borderRadius:10,padding:12,marginTop:8}}>
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}>
        <span style={{fontSize:16}}>☀</span>
        <span style={{fontFamily:"'Syne',sans-serif",fontSize:10,letterSpacing:".18em",textTransform:"uppercase",color:"#FBBF24",fontWeight:600}}>Israel Sun Calculator</span>
      </div>
      <div className="g3" style={{marginBottom:10}}>
        <div className="of-wrap">
          <div className="of-label">City</div>
          <select className="of-input" value={city} onChange={e=>setCity(e.target.value)} style={{cursor:"pointer"}}>
            {Object.keys(ISRAEL_CITIES).map(c=><option key={c}>{c}</option>)}
          </select>
        </div>
        <div className="of-wrap">
          <div className="of-label">Month</div>
          <select className="of-input" value={month} onChange={e=>setMonth(parseInt(e.target.value))} style={{cursor:"pointer"}}>
            {MONTHS.map((m,i)=><option key={m} value={i+1}>{m}</option>)}
          </select>
        </div>
        <div className="of-wrap">
          <div className="of-label">Hour</div>
          <select className="of-input" value={hour} onChange={e=>setHour(parseInt(e.target.value))} style={{cursor:"pointer"}}>
      ...
      <div style={{display:"flex",gap:8,marginBottom:10,flexWrap:"wrap"}}>
        {[
          {label:"Elevation",val:isNight?"Below horizon":sun.elevation+"°",color:"#FBBF24"},
          {label:"Azimuth",val:sun.azimuth+"°",color:"#A78BFA"},
          {label:"Shadow",val:isNight?"N/A":sun.shadowLen+"x height",color:"#5EEAD4"},
          {label:"Color Temp",val:sun.colorTemp+"K",color:"#F472B6"},
        ].map(({label,val,color})=>(
          <div key={label} style={{flex:1,minWidth:80,padding:"6px 8px",background:"rgba(0,0,0,0.2)",borderRadius:7,textAlign:"center"}}>
            <div style={{fontSize:9,color:"#6A6585",fontFamily:"'Syne',sans-serif",letterSpacing:".12em",textTransform:"uppercase",marginBottom:2}}>{label}</div>
            <div style={{fontSize:13,fontWeight:700,color,fontFamily:"'Syne',sans-serif"}}>{val}</div>
          </div>
        ))}
      </div>
      <button className="btn btn-s" style={{width:"100%",fontSize:10}} onClick={apply}>
        ↓ Apply to Lighting
      </button>
      {sunData.city && <div style={{marginTop:6,fontSize:10,color:"#6A6585",textAlign:"center"}}>Applied: {sunData.city} · {MONTHS[sunData.month-1]} · {sunData.hour}:00</div>}
    </div>
  );
}

// ─── CONTEXT ENGINE COMPONENT ─────────────────────────────
function ContextEngine({ onApply, t }) {
  const [selected, setSelected] = useState(null);
  return (
    <div style={{marginBottom:14}}>
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10,paddingBottom:8,borderBottom:"1px solid var(--bdr)"}}>
        <span style={{fontSize:12}}>🌍</span>
        <span style={{fontFamily:"'Syne',sans-serif",fontSize:10,letterSpacing:".2em",textTransform:"uppercase",color:"var(--p)",fontWeight:600}}>Context Engine</span>
        <span style={{fontSize:9,color:"var(--mut)",marginLeft:"auto"}}>One click → full coherent environment</span>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
        {Object.entries(CONTEXT_PROFILES).map(([key,prof])=>(
          <button key={key}
            onClick={()=>{setSelected(key); onApply(prof);}}
            style={{padding:"9px 10px",border:"1px solid",borderColor:selected===key?"var(--p)":"var(--bdr)",borderRadius:9,background:selected===key?"var(--pg)":"var(--pg2)",cursor:"pointer",textAlign:"left",transition:"all .2s"}}>
            <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:3}}>
              <span style={{fontSize:16}}>{prof.icon}</span>
              <span style={{fontFamily:"'Syne',sans-serif",fontSize:10,fontWeight:700,color:selected===key?"var(--pl)":"var(--txt)",letterSpacing:".04em"}}>{prof.label}</span>
            </div>
            <div style={{fontSize:9,color:"var(--mut)",lineHeight:1.4}}>{prof.neighborBuildings.slice(0,60)}...</div>
          </button>
        ))}
      </div>
    </div>
...
// ─── SHOT DIRECTOR COMPONENT ──────────────────────────────
function ShotDirector({ onApply, t }) {
  return (
    <div style={{marginBottom:14}}>
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10,paddingBottom:8,borderBottom:"1px solid var(--bdr)"}}>
        <span style={{fontSize:12}}>🎬</span>
        <span style={{fontFamily:"'Syne',sans-serif",fontSize:10,letterSpacing:".2em",textTransform:"uppercase",color:"var(--p)",fontWeight:600}}>Shot Director</span>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:5}}>
        {Object.entries(SHOT_TYPES).map(([key,shot])=>(
          <button key={key}
            onClick={()=>onApply(shot)}
            style={{padding:"8px 6px",border:"1px solid var(--bdr)",borderRadius:8,background:"var(--pg2)",cursor:"pointer",textAlign:"center",transition:"all .2s"}}>
            <div style={{fontSize:18,marginBottom:3}}>{shot.icon}</div>
            <div style={{fontFamily:"'Syne',sans-serif",fontSize:9,fontWeight:700,color:"var(--pl)",letterSpacing:".05em",marginBottom:2}}>{shot.label}</div>
            <div style={{fontSize:8,color:"var(--mut)",lineHeight:1.3}}>{shot.desc}</div>
          </button>
        ))}
      </div>
    </div>
...
// ─── GROUND FLOOR PANEL ───────────────────────────────────
function GroundFloorPanel({ gf, setGf, t }) {
  const heightOptions = [
    {key:"single", label:"Single", m:"3.0m", desc:"Standard"},
    {key:"high",   label:"High",   m:"4.5m", desc:"Elevated"},
    {key:"double", label:"Double", m:"6.0m", desc:"Double height"},
    {key:"custom", label:"Custom", m:"", desc:"Manual"},
  ];
  return (
    <div style={{display:"flex",flexDirection:"column",gap:10}}>
      {/* Height selector */}
      <div>
        <div className="of-label" style={{marginBottom:7}}>Ground Floor Height</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:5,marginBottom:8}}>
          {heightOptions.map(h=>(
            <button key={h.key} onClick={()=>setGf(p=>({...p,heightMode:h.key,height:h.m||p.customHeight||"3.5m"}))}
              style={{padding:"9px 5px",border:"1px solid",borderColor:gf.heightMode===h.key?"var(--p)":"var(--bdr)",borderRadius:8,background:gf.heightMode===h.key?"var(--pg)":"var(--pg2)",cursor:"pointer",textAlign:"center",transition:"all .2s"}}>
              <div style={{fontFamily:"'Syne',sans-serif",fontSize:12,fontWeight:700,color:gf.heightMode===h.key?"var(--pl)":"var(--txt)",marginBottom:2}}>{h.label}</div>
              <div style={{fontSize:9,color:"var(--mut)"}}>{h.m||"—"}</div>
              <div style={{fontSize:8,color:"var(--mut)"}}>{h.desc}</div>
            </button>
          ))}
        </div>
        {gf.heightMode==="custom" && (
          <div style={{display:"flex",alignItems:"center",gap:8,padding:"7px 10px",background:"var(--pg2)",border:"1px solid var(--bdr)",borderRadius:8}}>
            <span style={{fontSize:11,color:"var(--mut)"}}>Height:</span>
            <input type="number" min="2.5" max="15" step="0.1"
              value={gf.customHeight||""}
              onChange={e=>setGf(p=>({...p,customHeight:e.target.value,height:e.target.value+"m"}))}
              placeholder="e.g. 5.4"
              style={{width:70,background:"transparent",border:"1px solid var(--bdr)",borderRadius:6,padding:"4px 8px",color:"var(--txt)",fontSize:13,fontFamily:"'Syne',sans-serif",fontWeight:700,textAlign:"center",outline:"none"}}/>
            <span style={{fontSize:11,color:"var(--mut)"}}>meters</span>
            {gf.customHeight && <span style={{fontSize:11,color:"var(--pl)",fontFamily:"'Syne',sans-serif",fontWeight:600}}>{gf.customHeight}m floor-to-ceiling</span>}
          </div>
        )}
        {gf.heightMode && gf.heightMode!=="custom" && (
          <div style={{fontSize:10,color:"var(--pl)",padding:"4px 8px",background:"var(--pg2)",borderRadius:6,display:"inline-block",fontFamily:"'Syne',sans-serif"}}>
            {gf.height} floor-to-ceiling height
          </div>
        )}
      </div>
      <OptionField label="Ground Floor Type" value={gf.type} onChange={v=>setGf(p=>({...p,type:v}))} options={GROUND_FLOOR_TYPES}/>
      <OptionField label="Colonnade" value={gf.colonnade} onChange={v=>setGf(p=>({...p,colonnade:v}))} options={COLONNADE_TYPES}/>
      <OptionField label="Signage & Retail" value={gf.signage} onChange={v=>setGf(p=>({...p,signage:v}))} options={GROUND_SIGNAGE}/>
      <div className="tog-row"><span className="tog-lbl">Interior lighting visible</span><Toggle v={gf.lightingOn} set={v=>setGf(p=>({...p,lightingOn:v}))}/></div>
      {gf.lightingOn && <OptionField label="Lighting Type" value={gf.interiorLight} onChange={v=>setGf(p=>({...p,interiorLight:v}))} options={INTERIOR_LIGHT_TYPES}/>}
      <div className="tog-row"><span className="tog-lbl">Colonnade planting (hanging greenery)</span><Toggle v={gf.planting} set={v=>setGf(p=>({...p,planting:v}))}/></div>
      <div className="tog-row"><span className="tog-lbl">Visible interior depth through glass</span><Toggle v={gf.interiorDepth} set={v=>setGf(p=>({...p,interiorDepth:v}))}/></div>
    </div>
  );
}

// ─── RAL COLOR PICKER COMPONENT ──────────────────────────
function RALColorPicker({ value, hexValue, onChange, label }) {
  const [showAll, setShowAll] = useState(false);
  // Find closest RAL name from hex
  const findClosestRal = (hex) => {
    if (!hex) return null;
    const r1=parseInt(hex.slice(1,3),16), g1=parseInt(hex.slice(3,5),16), b1=parseInt(hex.slice(5,7),16);
    let closest = null, minDist = Infinity;
    RAL_PRESETS.forEach(ral => {
      const r2=parseInt(ral.hex.slice(1,3),16), g2=parseInt(ral.hex.slice(3,5),16), b2=parseInt(ral.hex.slice(5,7),16);
      const dist = Math.sqrt((r1-r2)**2+(g1-g2)**2+(b1-b2)**2);
      if (dist < minDist) { minDist=dist; closest=ral; }
    });
    return minDist < 60 ? closest : null;
  };
  const closestRal = hexValue ? findClosestRal(hexValue) : null;
  return (
    <div>
      <div className="of-label" style={{marginBottom:8}}>{label}</div>
      {/* Color wheel + hex input row */}
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10,padding:"10px",background:"var(--pg2)",border:"1px solid var(--bdr)",borderRadius:10}}>
        <div style={{position:"relative"}}>
          <input type="color" value={hexValue||"#F4F0E4"}
            onChange={e=>{
              const hex=e.target.value;
              const closest=findClosestRal(hex);
              onChange(closest?closest.code:"Custom", hex);
            }}
            style={{width:52,height:52,border:"2px solid var(--bdr)",borderRadius:10,cursor:"pointer",padding:2,background:"none",display:"block"}}/>
          <div style={{position:"absolute",bottom:-4,right:-4,width:16,height:16,borderRadius:"50%",background:hexValue||"#F4F0E4",border:"2px solid rgba(255,255,255,0.3)",boxShadow:"0 0 6px rgba(0,0,0,0.4)"}}/>
        </div>
        <div style={{flex:1}}>
          <div style={{display:"flex",gap:6,marginBottom:5}}>
            <input className="of-input" value={hexValue||""}
              onChange={e=>{
                const hex=e.target.value;
                if(/^#[0-9A-Fa-f]{0,6}$/.test(hex)){
                  const closest=hex.length===7?findClosestRal(hex):null;
                  onChange(closest?closest.code:"Custom",hex);
                }
              }}
              style={{flex:1,fontFamily:"monospace",fontSize:12}} placeholder="#F4F0E4"/>
            <div style={{padding:"6px 10px",background:"var(--pg)",border:"1px solid var(--p)",borderRadius:7,fontFamily:"'Syne',sans-serif",fontSize:10,fontWeight:700,color:"var(--pl)",whiteSpace:"nowrap"}}>
              {value||"RAL 9010"}
            </div>
          </div>
          {closestRal && (
            <div style={{fontSize:10,color:"var(--pl)"}}>≈ {closestRal.code} — {closestRal.name}</div>
          )}
          {!closestRal && hexValue && hexValue.length===7 && (
            <div style={{fontSize:10,color:"var(--mut)"}}>Custom color (no RAL match)</div>
          )}
        </div>
      </div>
      {/* RAL swatches */}
      <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
        {(showAll ? RAL_PRESETS : RAL_PRESETS.slice(0,8)).map(ral=>(
          <button key={ral.code} onClick={()=>onChange(ral.code,ral.hex)}
            title={ral.code+" — "+ral.name}
            style={{display:"flex",flexDirection:"column",alignItems:"center",gap:2,padding:"4px 5px",border:"1px solid",borderColor:value===ral.code?"var(--p)":"var(--bdr)",borderRadius:7,background:value===ral.code?"var(--pg)":"var(--pg2)",cursor:"pointer",transition:"all .15s",minWidth:44}}>
            <div style={{width:28,height:28,borderRadius:5,background:ral.hex,border:"1px solid rgba(255,255,255,0.12)",boxShadow:value===ral.code?"0 0 8px var(--pg)":"none"}}/>
            <span style={{fontSize:7.5,fontFamily:"'Syne',sans-serif",fontWeight:600,color:value===ral.code?"var(--pl)":"var(--mut)",lineHeight:1}}>{ral.code}</span>
          </button>
        ))}
        <button onClick={()=>setShowAll(p=>!p)}
          style={{padding:"4px 7px",border:"1px dashed var(--bdr)",borderRadius:7,background:"transparent",cursor:"pointer",fontSize:9,color:"var(--mut)",fontFamily:"'Syne',sans-serif",alignSelf:"center"}}>
          {showAll?"Less ▲":"More ▼"}
        </button>
      </div>
    </div>
...
function MaterialSpecsPanel({ ms, setMs, t }) {
  return (
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      <RALColorPicker
        label="Facade Panel Color (RAL)"
        value={ms.facadeRal}
        hexValue={ms.facadeHex}
        onChange={(ral,hex)=>setMs(p=>({...p,facadeRal:ral,facadeHex:hex}))}
      />
      <RALColorPicker
        label="Window Frame Color (RAL)"
        value={ms.frameRal}
        hexValue={ms.frameHex||"#5B6272"}
        onChange={(ral,hex)=>setMs(p=>({...p,frameRal:ral,frameHex:hex}))}
      />
      <RALColorPicker
        label="Railing / Secondary Element (RAL)"
        value={ms.railingRal}
        hexValue={ms.railingHex||"#ECF0F1"}
        onChange={(ral,hex)=>setMs(p=>({...p,railingRal:ral,railingHex:hex}))}
      />
      <OptionField label="Glass Type" value={ms.glassType} onChange={v=>setMs(p=>({...p,glassType:v}))} options={GLASS_TYPES.map(g=>g.label)}/>
      <OptionField label="Frame Profile" value={ms.frameProfile} onChange={v=>setMs(p=>({...p,frameProfile:v}))} options={["Ultra-thin 10mm aluminum","Standard 50mm aluminum","Heavy 80mm steel","Flush no-frame","Timber 60mm"]}/>
      <OptionField label="Construction System" value={ms.constructionSystem} onChange={v=>setMs(p=>({...p,constructionSystem:v}))} options={["Cast-in-place concrete (שטפנל)","Brnowitz system — insulated cast concrete","ECOWALL — lightweight steel frame","Curtain wall — aluminum + glass","Precast concrete panels","Masonry — Jerusalem stone cladding","Post + beam — steel frame + cladding"]}/>
      <div className="tog-row"><span className="tog-lbl">Strict model fidelity — no design interpretation</span><Toggle v={ms.strictFidelity} set={v=>setMs(p=>({...p,strictFidelity:v}))}/></div>
    </div>
  );
}

// ─── IMPERFECTIONS PANEL ─────────────────────────────────
function ImperfectionsPanel({ level, setLevel, custom, setCustom, t }) {
  return (
    <div style={{display:"flex",flexDirection:"column",gap:10}}>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
        {Object.entries(IMPERFECTION_LEVELS).map(([key,imp])=>(
          <button key={key} onClick={()=>setLevel(key)}
            style={{padding:"9px 8px",border:"1px solid",borderColor:level===key?"var(--p)":"var(--bdr)",borderRadius:9,background:level===key?"var(--pg)":"var(--pg2)",cursor:"pointer",textAlign:"left",transition:"all .2s"}}>
            <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:3}}>
              <span style={{fontSize:14}}>{imp.icon}</span>
              <span style={{fontFamily:"'Syne',sans-serif",fontSize:10,fontWeight:700,color:level===key?"var(--pl)":"var(--txt)"}}>{imp.label}</span>
            </div>
            <div style={{fontSize:9,color:"var(--mut)"}}>{imp.desc}</div>
          </button>
        ))}
      </div>
      {level !== "none" && (
        <div style={{padding:"8px 10px",background:"var(--pg2)",border:"1px solid var(--bdr)",borderRadius:8,fontSize:10,color:"var(--pl)",lineHeight:1.6}}>
          {IMPERFECTION_LEVELS[level].text}
        </div>
      )}
      <OptionField label="Additional imperfection details" value={custom} onChange={setCustom} options={["Visible AC units and pipes on side facade","Water stains on concrete elements","Rust streaks from steel fixings","Graffiti at ground level","Bird netting on ledges","Repair patches on facade"]} multiline/>
    </div>
  );
}

// ─── PHOTOGRAPHIC PANEL ──────────────────────────────────
function PhotoPanel({ photo, setPhoto, t }) {
  return (
    <div style={{display:"flex",flexDirection:"column",gap:10}}>
      <div className="g2">
        <OptionField label="Aperture" value={photo.aperture} onChange={v=>setPhoto(p=>({...p,aperture:v}))} options={["f/1.8 — very shallow DoF","f/2.8 — shallow DoF","f/5.6 — moderate DoF","f/8 — deep DoF (architectural)","f/11 — maximum sharpness","f/16 — hyperfocal"]}/>
        <OptionField label="Shutter / Motion" value={photo.shutter} onChange={v=>setPhoto(p=>({...p,shutter:v}))} options={MOTION_BLUR_OPTIONS}/>
      </div>
      <div className="g2">
        <OptionField label="Color Grade" value={photo.colorGrade} onChange={v=>setPhoto(p=>({...p,colorGrade:v}))} options={["Natural — no grading, RAW look","Slightly warm — 100K shift","Cool editorial — slight desaturation","High contrast — architectural drama","Flat neutral — maximum detail"]}/>
        <OptionField label="Demographics" value={photo.demographic} onChange={v=>setPhoto(p=>({...p,demographic:v}))} options={DEMOGRAPHIC_OPTIONS}/>
      </div>
      <div className="tog-row"><span className="tog-lbl">Chromatic aberration (very subtle)</span><Toggle v={photo.chromaticAb} set={v=>setPhoto(p=>({...p,chromaticAb:v}))}/></div>
      <div className="tog-row"><span className="tog-lbl">Sensor noise in shadows (subtle)</span><Toggle v={photo.sensorNoise} set={v=>setPhoto(p=>({...p,sensorNoise:v}))}/></div>
      <div className="tog-row"><span className="tog-lbl">Atmospheric haze (depth separation)</span><Toggle v={photo.atmoHaze} set={v=>setPhoto(p=>({...p,atmoHaze:v}))}/></div>
      <div className="tog-row"><span className="tog-lbl">Natural lens softness at edges</span><Toggle v={photo.lensSoftness} set={v=>setPhoto(p=>({...p,lensSoftness:v}))}/></div>
    </div>
  );
}

function buildExteriorPrompt(s, buildings, count, podium, cf, plat, prog, iCtx, negPr, styleRefs, weights, season, orientation, smartSun, gf, ms, impLevel, impCustom, photo, sunData, programFloors) {
  ...
  const bLines = buildings.slice(0, count).map((b, i) => { const tier = HEIGHT_TIERS[b.tier]; const floorsLabel = b.customFloors ? b.customFloors+"-story building" : tier.label+"-rise ("+tier.floors+")"; const pos = count===1 ? "The building" : count===2 ? (i===0?"LEFT building":"RIGHT building") : "Building "+b.name; return pos+" — "+floorsLabel+": "+b.shape+". Facade: "+b.facade+"."; });
  ...
  // Material specs
  const matSpec = ms ? [
    ms.strictFidelity ? "STRICT GEOMETRIC FIDELITY: No design interpretation. Exact massing and proportions must be preserved. DO NOT ADD NEW MASS OR BUILDINGS." : "",
    ms.constructionSystem ? "Construction system: "+ms.constructionSystem+"." : "",
    ms.facadeRal ? "Facade panels: "+ms.facadeRal+(ms.facadeHex?" ("+ms.facadeHex+")":",")+" matte finish, soft diffuse reflection, non-plastic appearance." : "",
    ms.glassType ? "Glass: "+(GLASS_TYPES.find(g=>g.label===ms.glassType)||{value:ms.glassType}).value+"." : "",
    ms.frameProfile ? "Window frames: "+ms.frameProfile+(ms.frameRal?", "+ms.frameRal:"")+"." : "",
  ].filter(Boolean).join(" ") : "";

  // Ground floor
  const gfSection = gf && gf.type ? [
    "", "GROUND FLOOR:",
    gf.type+(gf.height?", "+gf.height:"")+".",
    gf.colonnade && gf.colonnade!=="No colonnade — flush facade" ? "Colonnade: "+gf.colonnade+(gf.planting?", with hanging trailing plants (Silver Falls, Creeping Thyme, Trailing Rosemary) — lush colonnade garden.":".") : "",
    gf.signage ? "Signage: "+gf.signage+"." : "",
    gf.lightingOn && gf.interiorLight ? "Interior lighting: "+gf.interiorLight+"." : (gf.lightingOn===false ? "No artificial interior lighting — dark facade." : ""),
    gf.interiorDepth ? "Interior depth visible through glass — life inside, shadows and reflections." : "",
  ].filter(Boolean) : [];

  // Imperfections
  const impSection = impLevel && impLevel!=="none" ? [
    "", "REAL-WORLD IMPERFECTIONS:",
    IMPERFECTION_LEVELS[impLevel].text+(impCustom?". "+impCustom:"")+".",
  ] : [];

  // Sun / lighting precision
  const sunSection = sunData && sunData.city ? [
    "Color temperature: "+sunData.colorTemp+".",
    "Shadow length: "+sunData.shadowLen+".",
  ] : [];

  // Photographic
  const photoSection = photo ? [
    photo.aperture ? photo.aperture.split("—")[0].trim()+"." : "",
    photo.shutter ? "Shutter: "+photo.shutter+"." : "",
    photo.colorGrade ? "Color grade: "+photo.colorGrade+"." : "",
    photo.demographic ? "People: "+photo.demographic+"." : "",
    photo.chromaticAb ? "Subtle chromatic aberration." : "",
    photo.sensorNoise ? "Subtle sensor noise in shadow areas." : "",
    photo.atmoHaze ? "Soft atmospheric haze for depth separation." : "",
    photo.lensSoftness ? "Natural lens softness falloff toward edges." : "",
  ].filter(Boolean).join(" ") : "";

  const lines = [
    "Ultra-photorealistic architectural photography of "+count+" "+p.label.toLowerCase()+" building"+(count>1?"s":"")+" in "+s.location+".",
    "Program: "+p.label+(programFloors&&programFloors.length?" — floors: "+programFloors.join(", "):"")+".",
    matSpec ? "\n"+matSpec : "",
    "", "BUILDINGS:", ...bLines, "Base: "+podium+".", extra("buildings"),
    ...gfSection,
    "", "MATERIALS:", s.frame+" structural frame, "+s.glazing+", "+s.railings+" balcony railings.", extra("materials"),
    "", "BIOPHILIC GREENERY:", gr+".", extra("greenery"),
    "", "URBAN SETTING:", s.streetContext+", "+s.background+". "+s.foreground+"."+israelCtx+neighborSection+activitySection, extra("setting"),
    ...impSection,
    "", "LIGHTING & ATMOSPHERE:", s.timeOfDay+" at "+smartSun+(s.shadows?", dramatic shadow play across facade":"")+". "+s.skyDescription+"."+seaInfo, ...sunSection, extra("lighting"),
    "", "CAMERA & FRAMING:", s.cameraAngle+", "+s.lens+" lens, "+s.tilt+", "+s.composition+(s.dof?", shallow DoF":"")+(orientation?" Building faces "+orientation+".":""), extra("camera"),
    photoSection ? "\n"+photoSection : "",
    "", "RENDER SPECIFICATION:", s.renderer+", "+s.resolution+(s.filmGrain?", subtle film grain":"")+(s.chromaticAtm?", chromatic atmosphere":"")+", "+s.style+", shot on "+s.camera+".", extra("quality"),
    neg, refs,
  ];
...
function buildCompactPrompt(s, buildings, count, podium, plat, prog, negPr, styleRefs, season, weights, smartSun, renderMode) {
  ...
  const bDesc = buildings.slice(0,count).map(b => (b.customFloors ? b.customFloors+"-story" : HEIGHT_TIERS[b.tier].label+"-rise")+" "+b.shape+", "+b.facade).join(" | ");
...
export default function App() {
  ...
  const [programFloors, setProgramFloors] = useState([]);
  ...
  // New feature states
  const [gf, setGf] = useState({type:"Standard lobby entrance",height:"3.0m standard",colonnade:"No colonnade — flush facade",signage:"",lightingOn:true,interiorLight:"Warm residential 2700K — life visible",planting:false,interiorDepth:true});
  const [ms, setMs] = useState({facadeRal:"RAL 9010",facadeHex:"#F4F0E4",glassType:"",frameProfile:"Ultra-thin 10mm aluminum",frameRal:"RAL 9010 White",constructionSystem:"",strictFidelity:false});
  const [impLevel, setImpLevel] = useState("light");
  const [impCustom, setImpCustom] = useState("");
  const [photo, setPhoto] = useState({aperture:"f/8 — deep DoF (architectural)",shutter:"Slight motion — 1/250s",colorGrade:"Natural — no grading, RAW look",demographic:"Mixed age — authentic neighborhood",chromaticAb:true,sensorNoise:true,atmoHaze:true,lensSoftness:true});
  ...
  const prompt = compact ? buildCompactPrompt(s,buildings,count,podium,plat,programType,negPr,styleRefs,season,weights,smartSun,renderMode) : renderMode==="interior" ? buildInteriorPrompt(s,plat,programType,negPr) : renderMode==="plan" ? buildPlanPrompt(s,plat,programType) : buildExteriorPrompt(s,buildings,count,podium,cf,plat,programType,israeliCtx,negPr,styleRefs,weights,season,orientation,smartSun,gf,ms,impLevel,impCustom,photo,sunData,programFloors);
  ...
  const applyContext = (prof) => {
    setS(prev => ({...prev,
      location: prof.location, streetContext: prof.streetContext,
      background: prof.background, foreground: prof.foreground,
      skyDescription: prof.skyDescription,
    }));
  };
  const applyShot = (shot) => {
    setS(prev => ({...prev,
      cameraAngle: shot.angle.split(",")[0],
      lens: (shot.angle.match(/\d+mm/) || ["35mm"])[0],
      composition: shot.comp,
    }));
  };
          ...
          <div className={"left" + (drawerOpen?" open":"")}>
            ...
            <div className="left-content">
              {leftTab==="Build" && (<div>
                ...
                <div className="sec">
                  <div className="sec-hdr" style={{cursor:"default"}}><span className="sec-icon">🏗</span><span className="sec-lbl">{tr.secProgram}</span><span className="sec-line"/></div>
                  <div className="prog-grid" style={{marginBottom:12}}>
                    {Object.entries(PROGRAM_TYPES).map(([k,p]) => (
                      <button key={k} className={"prog-btn"+(programType===k?" on":"")} onClick={() => { setProgramType(k); setProgramFloors([]); }}>
                  ...
                  <div>
                    <div className="of-label" style={{marginBottom:6}}>Floor Uses — select what's in the building</div>
                    <div className="chips-plain" style={{gap:5}}>
                      {(FLOOR_USE_PRESETS[programType]||[]).map(use => (
                        <button key={use} className={"chip-p"+(programFloors.includes(use)?" on":"")}
                          onClick={() => setProgramFloors(prev => prev.includes(use) ? prev.filter(x=>x!==use) : [...prev, use])}
                          style={{marginBottom:4}}>
                          {use}
                        </button>
                      ))}
                    </div>
                    {programFloors.length > 0 && (
                      <div style={{marginTop:8,padding:"7px 10px",background:"var(--pg2)",border:"1px solid var(--bdr)",borderRadius:8,fontSize:10,color:"var(--pl)",lineHeight:1.6}}>
                        ✓ {programFloors.join(" · ")}
                      </div>
                    )}
                  </div>
                ...
                {renderMode==="exterior" && (<div>
                  <BuildingConfig buildings={buildings} setBuildings={setBuildings} count={count} setCount={setCount} mode={mode} setMode={setMode} podium={podium} setPodium={setPodium} cf={cf} setCF={setCF} isDark={isDark} t={t} tr={tr}/>
                ...
                <Section label={tr.secLighting} icon="☀" cfKey="lighting" cf={cf} setCF={setCF}>
                  ...
                  {renderMode==="exterior" && <SunCalculator sunData={sunData} setSunData={setSunData} t={tr}/>}
                </Section>
