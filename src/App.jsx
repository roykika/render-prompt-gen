...
function BuildingConfig({ buildings, setBuildings, count, setCount, mode, setMode, podium, setPodium, cf, setCF, isDark, t, tr }) {
  ...
  return (
    <div className="sec">
      ...
      {mode === "all" && (<div className="uniform-card"><div className="of-label" style={{marginBottom:8}}>Shared — all {count} building{count>1?"s":""}</div><BuildingForm b={uniform} onChange={b => { setUniform(b); setBuildings(p => p.map(x => ({...x, tier:b.tier, shape:b.shape, facade:b.facade}))); }} isDark={isDark} tr={tr}/></div>)}
