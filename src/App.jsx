...
.sec-lbl{font-family:'Syne',sans-serif;font-size:10px;letter-spacing:.22em;text-transform:uppercase;color:${t.purple};font-weight:600;transition:color .2s;flex:none}
...
.of-label{font-size:10px;letter-spacing:.18em;text-transform:uppercase;color:${t.muted};font-weight:500;font-family:'Syne',sans-serif;margin-bottom:5px}
.of-wrap{display:flex;flex-direction:column;gap:5px}
.of-input,.of-ta{width:100%;background:${t.inputBg};border:1px solid ${t.border};color:${t.text};font-family:'DM Sans',sans-serif;font-size:12px;font-weight:300;padding:7px 10px;outline:none;transition:all .2s;border-radius:8px}
...
.of-opt{padding:3px 9px;font-size:11px;border:1px solid ${t.border};cursor:pointer;transition:all .18s;background:${t.optBg};color:${t.muted};font-family:'DM Sans',sans-serif;border-radius:20px;line-height:1.4}
...
function ThemeFactoryPanel({ accent, setAccent, isDark, setIsDark }) {
  return (
    <div>
      <div className="sec-hdr" style={{cursor:"default", marginBottom:14}}><span className="sec-icon">🎨</span><span className="sec-lbl">Theme Factory</span><span className="sec-line"/></div>
      <div className="sec">
        <div className="of-label" style={{marginBottom:10}}>Mode</div>
        <div style={{display:"flex", gap:7, marginBottom:18}}>
          <button className={"btn-sm"+(isDark?" active":"")} style={{flex:1}} onClick={() => setIsDark(true)}>◑ Dark</button>
          <button className={"btn-sm"+(!isDark?" active":"")} style={{flex:1}} onClick={() => setIsDark(false)}>☀ Light</button>
        </div>
        <div className="of-label" style={{marginBottom:10}}>Accent Color Presets</div>
        ...
        <div className="of-label" style={{marginBottom:8}}>Custom Accent Color</div>
...
function CustomFields({ fields, onChange }) {
  ...
  const confirm = () => { if (!lbl.trim()) return; onChange([...fields, {id: ++uid, label: lbl.trim(), value: val}]); setLbl(""); setVal(""); setAdding(false); };
  ...
  return (
    <div className="cf-area">
      ...
      {adding
        ? <div className="af-wrap"><input className="af-input" style={{width:85}} placeholder="Label" value={lbl} onChange={e => setLbl(e.target.value)} autoFocus onKeyDown={e => {if(e.key==="Enter")confirm(); if(e.key==="Escape")setAdding(false);}}/><input className="af-input" style={{flex:1}} placeholder="Value" value={val} onChange={e => setVal(e.target.value)} onKeyDown={e => {if(e.key==="Enter")confirm(); if(e.key==="Escape")setAdding(false);}}/><button className="af-ok" onClick={confirm}>Add</button><button className="af-cancel" onClick={() => setAdding(false)}>✕</button></div>
        : <button className="add-btn" onClick={() => setAdding(true)}>⊕ Add Custom Field</button>}
    </div>
  );
}
...
function BuildingForm({ b, onChange, isDark }) {
  ...
  return (<div style={{display:"flex", flexDirection:"column", gap:10}}><div><div className="of-label" style={{marginBottom:7}}>Height Category</div><TierSelector value={b.tier} onChange={setTier} isDark={isDark}/></div><OptionField label="Shape / Form" value={b.shape} onChange={v => onChange({...b, shape:v})} options={tier.shapes}/><OptionField label="Facade Language" value={b.facade} onChange={v => onChange({...b, facade:v})} options={tier.facades} multiline/></div>);
}
...
function BuildingConfig({ buildings, setBuildings, count, setCount, mode, setMode, podium, setPodium, cf, setCF, isDark, t }) {
  ...
  return (
    <div className="sec">
      <div className="sec-hdr" style={{cursor:"default"}}><span className="sec-icon">🏢</span><span className="sec-lbl">Building Configuration</span><span className="sec-line"/></div>
      <div className="bc-controls">
        <div className="count-ctrl"><span className="count-lbl">Buildings</span><button className="count-step" onClick={() => updateCount(count-1)}>−</button><span className="count-num">{count}</span><button className="count-step" onClick={() => updateCount(count+1)}>+</button></div>
        <div className="mode-sw"><button className={"msw-btn" + (mode==="all"?" on":"")} onClick={() => setMode("all")}>All Same</button><button className={"msw-btn" + (mode==="individual"?" on":"")} onClick={() => setMode("individual")}>Individual</button></div>
      </div>
      ...
      <OptionField label="Shared Podium / Base" value={podium} onChange={setPodium} options={PRESETS.podium}/>
...
function TemplatesPanel({ applyTemplate }) {
  return (<div><div className="sec-hdr" style={{cursor:"default", marginBottom:14}}><span className="sec-icon">⚡</span><span className="sec-lbl">Quick-Start Templates</span><span className="sec-line"/></div><div className="tpl-grid">{TEMPLATES.map(tpl => (<button key={tpl.name} className="tpl-card" onClick={() => applyTemplate(tpl)}><div className="tpl-head"><span className="tpl-icon">{tpl.icon}</span><div><div className="tpl-name">{tpl.name}</div><span className="tpl-tag">{tpl.tag}</span></div></div><div className="tpl-desc">{tpl.desc}</div></button>))}</div></div>);
}
function PresetsPanel({ getSnapshot, applySnapshot }) {
  const [presets, setPresets] = useState([]);
  const [name, setName] = useState("");
  const save = () => { if (!name.trim()) return; const p = {name: name.trim(), ts: Date.now(), snapshot: getSnapshot()}; setPresets(u => [...u.filter(x => x.name !== p.name), p]); setName(""); };
  const del = n => setPresets(u => u.filter(x => x.name !== n));
  return (
    <div className="sec">
      <div className="sec-hdr" style={{cursor:"default"}}><span className="sec-icon">📁</span><span className="sec-lbl">Saved Presets</span><span className="sec-line"/>{presets.length > 0 && <span className="sec-count">{presets.length}</span>}</div>
      {presets.length === 0 ? <div className="empty-state"><div className="empty-icon">📂</div><div>No saved presets yet.</div></div> : <div className="preset-list">{presets.map(p => (<div key={p.name} className="preset-item"><div style={{flex:1}}><div className="preset-name">{p.name}</div><div className="preset-date">{new Date(p.ts).toLocaleDateString()}</div></div><div style={{display:"flex", gap:5}}><button className="pact-btn load" onClick={() => applySnapshot(p.snapshot)}>Load</button><button className="pact-btn del" onClick={() => del(p.name)}>×</button></div></div>))}</div>}
      <div className="save-row"><input className="of-input" style={{flex:1}} value={name} onChange={e => setName(e.target.value)} placeholder="Preset name..." onKeyDown={e => e.key==="Enter" && save()}/><button className="btn-sm active" onClick={save}>Save</button></div>
    </div>
  );
}
function HistoryPanel({ history, onSelect }) {
  if (!history.length) return (<div className="empty-state"><div className="empty-icon">🕐</div><div>No history yet.<br/>Start editing to track changes.</div></div>);
  return (<div className="sec"><div className="sec-hdr" style={{cursor:"default"}}><span className="sec-icon">🕐</span><span className="sec-lbl">History</span><span className="sec-line"/><span className="sec-count">{history.length}</span></div>{history.map((h, i) => (<div key={h.ts} className="hist-item" onClick={() => onSelect(h.prompt)} title="Click to copy"><div className="hist-time">{i===0?"● Now — ":""}{new Date(h.ts).toLocaleTimeString()}</div><div className="hist-preview">{h.prompt}</div></div>))}</div>);
}
function ComparePanel({ slotA, slotB, onClearA, onClearB }) {
  ...
  return (
    <div className="sec">
      <div className="sec-hdr" style={{cursor:"default"}}><span className="sec-icon">⚖</span><span className="sec-lbl">Compare</span><span className="sec-line"/></div>
      <div style={{fontSize:11, color:"#7A7590", marginBottom:10, lineHeight:1.5}}>Use "→ A" and "→ B" in the output panel.</div>
      <div className="compare-grid">
        {[{l:"Slot A",c:slotA,clr:onClearA},{l:"Slot B",c:slotB,clr:onClearB}].map(({l,c,clr}) => (<div key={l} className="compare-slot"><div className="compare-slot-hdr"><span className="compare-slot-lbl">{l}</span>{c && <button className="pact-btn del" onClick={clr}>Clear</button>}</div>{c ? <div className="compare-text">{c}</div> : <div className="compare-empty">Empty</div>}</div>))}
      </div>
      {lines && (<div style={{marginTop:10}}><div className="of-label" style={{marginBottom:6}}>Diff</div><div style={{background:"rgba(7,7,20,.85)", border:"1px solid #1E1E3A", borderRadius:8, padding:10, maxHeight:130, overflowY:"auto", fontSize:11, fontFamily:"'DM Sans',sans-serif", lineHeight:1.7}}>{lines.filter(l => l.changed).length === 0 ? <span style={{color:"#7A7590"}}>Identical</span> : lines.filter(l => l.changed).map((l, i) => (<div key={i} style={{marginBottom:3}}>{l.a && <div className="diff-remove">− {l.a}</div>}{l.b && <div className="diff-add">+ {l.b}</div>}</div>))}</div></div>)}
    </div>
  );
}
            ...
            <div className="left-content">
              {leftTab==="Build" && (<div>
                ...
                {renderMode==="exterior" && (<div>
                  ...
                  <Section label="Materials" icon="🧱" cfKey="materials" cf={cf} setCF={setCF}><div className="g3">{OF("Frame","frame")}{OF("Glazing","glazing")}{OF("Railings","railings")}</div></Section>
                  <Section label="Biophilic Greenery" icon="🌿" cfKey="greenery" cf={cf} setCF={setCF}><div style={{display:"flex",flexDirection:"column",gap:10}}><div><div className="of-label" style={{marginBottom:6}}>Greenery Types</div><PinnableChips field="greenery" options={GREENERY_OPTIONS} value={s.greeneryTypes} onChange={set("greeneryTypes")} pins={pins} togglePin={togglePin}/></div>{OF("Density / Placement","greeneryDensity")}</div></Section>
                  <Section label="Urban Setting" icon="🏙" cfKey="setting" cf={cf} setCF={setCF}><div className="g2">{OF("Location","location")}{OF("Street Context","streetContext")}{OF("Background","background")}{OF("Foreground","foreground")}</div><div style={{marginTop:11}}><div className="of-label" style={{marginBottom:6}}>Israeli Planning Context</div><PinnableChips field="israeliCtx" options={PRESETS.israeliContext} value={israeliCtx} onChange={setIsraeliCtx} pins={pins} togglePin={togglePin}/></div></Section>
                  <Collapsible label="Neighboring Context" icon="🏘"><div style={{display:"flex",flexDirection:"column",gap:10}}><OptionField label="Adjacent Buildings" value={s.neighborBuildings} onChange={set("neighborBuildings")} options={["Lower-rise residential surroundings","Similar-height office towers","Historic low-rise preservation zone","Mixed-height dense urban fabric","Open site — no significant neighbors","Waterfront with open views"]}/><OptionField label="Street Character" value={s.neighborStreet} onChange={set("neighborStreet")} options={["Wide 4-lane boulevard with mature trees","Narrow pedestrian lane","Tree-lined avenue","Coastal waterfront promenade","Park-adjacent open green"]}/></div></Collapsible>
                  <Collapsible label="People & Activity" icon="👥"><div style={{display:"flex",flexDirection:"column",gap:10}}><OptionField label="Crowd Density" value={s.crowdDensity} onChange={set("crowdDensity")} options={["Deserted — architectural focus","Sparse — few distant figures","Moderate everyday urban activity","Busy lively street scene","Evening social atmosphere"]}/><OptionField label="Activity Type" value={s.activityType} onChange={set("activityType")} options={["Families with children in plaza","Business professionals walking","Mixed casual urban crowd","Cyclists and joggers","Outdoor café terrace culture"]}/></div></Collapsible>
                </div>)}
                {renderMode==="interior" && (<div>
                  <Section label="Interior Space" icon="🏠" cfKey="quality" cf={cf} setCF={setCF}><div style={{display:"flex",flexDirection:"column",gap:10}}><OptionField label="Space Type" value={s.spaceType} onChange={set("spaceType")} options={INTERIOR_SPACES}/><div className="g2"><OptionField label="Ceiling Height" value={s.ceilingHeight} onChange={set("ceilingHeight")} options={CEIL_OPTIONS}/><OptionField label="Flooring" value={s.flooring} onChange={set("flooring")} options={FLOOR_OPTIONS}/></div><OptionField label="Lighting Scheme" value={s.interiorLighting} onChange={set("interiorLighting")} options={INT_LIGHT_OPTIONS}/><OptionField label="Mood & Atmosphere" value={s.intMood} onChange={set("intMood")} options={INT_MOOD_OPTIONS}/></div></Section>
                  <Section label="Materials" icon="🧱" cfKey="materials" cf={cf} setCF={setCF}><div className="g3">{OF("Wall / Structure","frame")}{OF("Glazing","glazing")}{OF("Details","railings")}</div></Section>
                  <Section label="Biophilic Elements" icon="🌿" cfKey="greenery" cf={cf} setCF={setCF}><div><div className="of-label" style={{marginBottom:6}}>Interior Plants</div><PinnableChips field="greenery" options={GREENERY_OPTIONS} value={s.greeneryTypes} onChange={set("greeneryTypes")} pins={pins} togglePin={togglePin}/></div></Section>
                ...
                {renderMode==="plan" && (<div>
                  <Section label="Plan Configuration" icon="📐" cfKey="quality" cf={cf} setCF={setCF}><div style={{display:"flex",flexDirection:"column",gap:10}}><OptionField label="View / Projection" value={s.planView} onChange={set("planView")} options={PLAN_VIEW_OPTIONS}/><OptionField label="Graphic Style" value={s.planStyle} onChange={set("planStyle")} options={PLAN_STYLE_OPTIONS}/><div className="g2"><OptionField label="Floor Level" value={s.planFloor} onChange={set("planFloor")} options={PLAN_FLOOR_OPTIONS}/><OptionField label="Scale" value={s.planScale} onChange={set("planScale")} options={PLAN_SCALE_OPTIONS}/></div><OptionField label="Color Scheme" value={s.planColorScheme} onChange={set("planColorScheme")} options={PLAN_COLOR_OPTIONS}/></div></Section>
                  <Section label="Plan Elements" icon="☑" cfKey="setting" cf={cf} setCF={setCF}><div><div className="of-label" style={{marginBottom:6}}>Shown Elements</div><PlainChips options={PLAN_ELEMENTS} value={s.planElements||[]} onChange={set("planElements")}/></div></Section>
                  <Section label="Surface Materials" icon="🧱" cfKey="materials" cf={cf} setCF={setCF}><div className="g2"><OptionField label="Floor Finish" value={s.flooring} onChange={set("flooring")} options={FLOOR_OPTIONS}/>{OF("Wall Material","frame")}</div></Section>
                ...
                <Section label="Season & Lighting" icon="☀" cfKey="lighting" cf={cf} setCF={setCF}>
                  {renderMode!=="plan" && (<div style={{marginBottom:11}}><div className="of-label" style={{marginBottom:6}}>Season / Weather</div><div className="season-grid">{Object.entries(SEASONS).map(([k,v]) => (<button key={k} className={"season-btn"+(season===k?" on":"")} onClick={() => { setSeason(k); set("skyDescription")(v.sky); set("sunAngle")(v.sun); }}>{v.label}</button>))}</div></div>)}
                  <div className="g2" style={{marginBottom:9}}>{renderMode!=="plan" && <OptionField label="Time of Day" value={s.timeOfDay} onChange={set("timeOfDay")} options={TIME_OPTIONS}/>}{OF("Sun Angle","sunAngle")}{renderMode!=="plan" && OF("Sky Description","skyDescription",false,true)}</div>
                  {renderMode!=="plan" && <div className="tog-row"><span className="tog-lbl">Dramatic shadow play</span><Toggle v={s.shadows} set={set("shadows")}/></div>}
                </Section>
                <Section label="Camera & Framing" icon="📷" cfKey="camera" cf={cf} setCF={setCF}>
                  {renderMode==="exterior" && (<div style={{marginBottom:11}}><div className="of-label" style={{marginBottom:7}}>Site Orientation</div><div className="compass-wrap"><CompassRose orientation={orientation} onChange={setOrientation} t={t}/><div className="smart-sun"><span style={{fontSize:13,flexShrink:0,marginTop:1}}>☀</span><div><div style={{fontSize:9,color:t.muted,marginBottom:3,fontFamily:"'Syne',sans-serif",letterSpacing:".1em",textTransform:"uppercase"}}>{orientation}-facing</div>{smartSun}</div></div></div></div>)}
                  <div className="g2" style={{marginBottom:9}}>{OF("Camera Angle","cameraAngle")}<OptionField label="Lens" value={s.lens} onChange={set("lens")} options={LENS_OPTIONS}/>{renderMode!=="plan" && OF("Tilt","tilt")}{OF("Composition","composition",false,renderMode==="plan")}</div>
                  {renderMode!=="plan" && <div className="tog-row"><span className="tog-lbl">Depth of Field</span><Toggle v={s.dof} set={set("dof")}/></div>}
                </Section>
                <Section label="Render Quality" icon="✦" cfKey="quality" cf={cf} setCF={setCF}>
                  <div className="g3" style={{marginBottom:9}}><OptionField label="Renderer" value={s.renderer} onChange={set("renderer")} options={PRESETS.renderer}/><OptionField label="Resolution" value={s.resolution} onChange={set("resolution")} options={RES_OPTIONS}/><OptionField label="Aspect Ratio" value={s.aspectRatio} onChange={set("aspectRatio")} options={ASPECT_OPTIONS}/><OptionField label="Camera Body" value={s.camera} onChange={set("camera")} options={PRESETS.camera} fullRow/><OptionField label="Style" value={s.style} onChange={set("style")} options={PRESETS.style} fullRow/></div>
                  {renderMode!=="plan" && (<div><div className="tog-row"><span className="tog-lbl">Film Grain</span><Toggle v={s.filmGrain} set={set("filmGrain")}/></div><div className="tog-row"><span className="tog-lbl">Chromatic Atmosphere</span><Toggle v={s.chromaticAtm} set={set("chromaticAtm")}/></div></div>)}
                </Section>
                {renderMode==="exterior" && (<Collapsible label="Style References" icon="🖼"><div style={{display:"flex",flexDirection:"column",gap:6}}><div className="of-label">Reference Image URLs</div>{styleRefs.map((ref,i) => (<input key={i} className="of-input" value={ref} onChange={e => setStyleRefs(p => p.map((r,j) => j===i?e.target.value:r))} placeholder={"Reference URL "+(i+1)+"..."}/>))}<button className="add-btn" onClick={() => setStyleRefs(p => [...p,""])}>⊕ Add Reference</button></div></Collapsible>)}
                <Collapsible label="Negative Prompts" icon="⊘"><PinnableChips field="negPr" options={PRESETS.negativePrompts} value={negPr} onChange={setNegPr} pins={pins} togglePin={togglePin}/></Collapsible>
                {plat==="midjourney" && renderMode==="exterior" && (<div className="sec"><div className="sec-hdr" style={{cursor:"default"}}><span className="sec-icon">⚖</span><span className="sec-lbl">Section Weights</span><span className="sec-line"/></div>{Object.entries(weights).map(([k,v]) => { const pct = ((v-0.5)/(2-0.5))*100; return (<div key={k} className="weight-row"><span className="weight-lbl">{k}</span><input type="range" className="weight-slider" min="0.5" max="2" step="0.1" value={v} style={{"--pct":pct+"%"}} onChange={e => setWeights(p => ({...p, [k]:parseFloat(e.target.value)}))}/><span className="weight-val">{v.toFixed(1)}</span></div>); })}</div>)}
              </div>)}
              {leftTab==="Templates" && <TemplatesPanel applyTemplate={applyTemplate}/>}
              {leftTab==="Presets" && <PresetsPanel getSnapshot={getSnapshot} applySnapshot={applySnapshot}/>}
              {leftTab==="History" && <HistoryPanel history={history} onSelect={p => doCopy(p)}/>}
              {leftTab==="Compare" && <ComparePanel slotA={compareA} slotB={compareB} onClearA={() => setCompareA(null)} onClearB={() => setCompareB(null)}/>}
              {leftTab==="Theme" && <ThemeFactoryPanel accent={accent} setAccent={setAccent} isDark={isDark} setIsDark={setIsDark}/>}
            </div>
          </div>
          <div className="out">
            <div><div className="out-lbl" style={{marginBottom:6}}>Platform</div><div className="ptabs">{PLATFORMS.map(p => (<button key={p.id} className={"ptab"+(plat===p.id?" on":"")} onClick={() => setPlat(p.id)}>{p.label}</button>))}</div>{plat==="gemini" && <div className="g-badge"><span className="g-dot"/>Gemini Imagen 3</div>}</div>
            <div><div className="out-lbl" style={{marginBottom:6}}>Mode</div><div className="out-toggles"><button className={"out-tog"+(!compact&&!tlMode?" on":"")} onClick={() => { setCompact(false); setTlMode(false); }}>Verbose</button><button className={"out-tog"+(compact&&!tlMode?" on":"")} onClick={() => { setCompact(true); setTlMode(false); }}>Compact</button>{renderMode==="exterior" && <button className={"out-tog"+(tlMode?" on":"")} onClick={() => { setTlMode(p => !p); setCompact(false); }}>⏱ x3</button>}<button className={"out-tog"+(colorMode?" on":"")} onClick={() => setColorMode(p => !p)}>🎨</button></div></div>
            <div className="out-lbl">Generated Prompt</div>
            ...
            <div className="btn-row"><button className="btn btn-p" onClick={copy}>{copied?"✓ Copied!":"Copy Prompt"}</button><button className="btn btn-s" onClick={exportPDF}>PDF</button><button className="btn btn-s" onClick={exportMarkdownFn} title="Copy as Markdown for Notion">MD</button><button className="btn btn-s" onClick={reset}>Reset</button></div>
            <div style={{display:"flex", gap:5}}><button className="btn-sm" style={{flex:1}} onClick={() => setCompareA(prompt)}>→ A</button><button className="btn-sm" style={{flex:1}} onClick={() => setCompareB(prompt)}>→ B</button><button className={"btn-sm"+(aiVisible?" active":"")} style={{flex:1}} onClick={() => setAiVisible(p => !p)}>🤖 AI</button><button className="btn-sm" style={{flex:1}} onClick={() => { setShareCode(genShareCode()); setShareVisible(p => !p); }}>🔗</button></div>
            <div className="copied-msg">{copied?"✓ Copied to clipboard":""}</div>
            {shareVisible && (<div className="share-panel"><div className="of-label">Share Code</div><div className="share-code-box">{shareCode}</div><div style={{display:"flex",gap:5}}><button className="btn-sm active" style={{flex:1}} onClick={() => doCopy(shareCode)}>Copy Code</button><button className="btn-sm" style={{flex:1}} onClick={() => setShareVisible(false)}>Close</button></div><div className="of-label" style={{marginTop:2}}>Load from Code</div><div style={{display:"flex",gap:5}}><input className="of-input" style={{flex:1,fontSize:11}} value={pasteCode} onChange={e => setPasteCode(e.target.value)} placeholder="Paste code..."/><button className="btn-sm active" onClick={() => loadFromCode(pasteCode)}>Load</button></div></div>)}
            {aiVisible && <AIPanel prompt={prompt} t={t} programType={PROGRAM_TYPES[programType].label} location={s.location}/>}
            <div className="quality-wrap"><div className="quality-row"><span style={{fontFamily:"'Syne',sans-serif",fontSize:9,letterSpacing:".18em",textTransform:"uppercase",color:t.muted,fontWeight:600}}>Prompt Quality</span><span style={{fontFamily:"'Syne',sans-serif",fontSize:9,letterSpacing:".16em",textTransform:"uppercase",color:quality.color,fontWeight:700}}>{quality.label}</span></div><div className="quality-track"><div className="quality-fill" style={{width:quality.pct+"%",background:quality.color}}/></div></div>
            <div className="stats">{[["Mode",RENDER_MODES[renderMode].icon],["Words",wordCount],["Custom",Object.values(cf).flat().length]].map(([l,v]) => (<div key={l} className="stat-card"><div className="stat-v">{v}</div><div className="stat-l">{l}</div></div>))}</div>
          </div>
        </div>
      </div>
    </div>
