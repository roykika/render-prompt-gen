...
function SavedPanel({ applyTemplate, getSnapshot, applySnapshot }) {
  ...
  return (
    <div>
      <div style={{display:"flex", borderBottom:"1px solid var(--bdr)", marginBottom:16}}>
        {[["templates","⚡ Templates"],["presets","📁 שמורים"]].map(([id,lbl]) => (
      ...
      {activeTab === "presets" && (
        <div>
          <div className="save-row" style={{marginBottom:14}}>
            <input className="of-input" style={{flex:1}} value={name} onChange={e => setName(e.target.value)} placeholder="שם הפריסט..." onKeyDown={e => e.key==="Enter" && save()}/>
            <button className="btn-sm active" onClick={save}>שמור</button>
          </div>
          {presets.length === 0
            ? <div className="empty-state"><div className="empty-icon">📂</div><div>אין פריסטים שמורים עדיין.<br/>שמור את ההגדרות הנוכחיות.</div></div>
            : <div className="preset-list">{presets.map(p => (
                <div key={p.name} className="preset-item">
                  <div style={{flex:1}}><div className="preset-name">{p.name}</div><div className="preset-date">{new Date(p.ts).toLocaleDateString("he-IL")}</div></div>
                  <div style={{display:"flex", gap:5}}>
                    <button className="pact-btn load" onClick={() => applySnapshot(p.snapshot)}>טען</button>
        ...
        <div className="layout">
          <div className={"left" + (drawerOpen?" open":"")}>
            <div className="left-tabs">
              {["Build","Saved","History","Compare","Theme"].map(tab => (<button key={tab} className={"ltab" + (leftTab===tab?" on":"")} onClick={() => { setLeftTab(tab); setDrawerOpen(false); }}>{tab}</button>))}
            </div>
            <div className="left-content">
              ...
              {leftTab==="Saved" && <SavedPanel applyTemplate={applyTemplate} getSnapshot={getSnapshot} applySnapshot={applySnapshot}/>}
          ...
          <div className="out">
            ...
            <div className="btn-row"><button className="btn btn-p" onClick={copy}>{copied?"✓ Copied!":"Copy Prompt"}</button><button className="btn btn-s" onClick={exportPDF}>PDF</button><button className="btn btn-s" onClick={reset}>Reset</button></div>
